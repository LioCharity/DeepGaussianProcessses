...
%optimize parameters
expX = optimize_hyperparameters( X,input,target,2 ); %the row 51 is now useless
...
for i=1:n
    for j=1:n %double loop to compute the derivative of the covariance matrix
        tmp=0;
        for d=1:D
            tmp = tmp + expX(d)*(input(i,d)-input(j,d))^2;
        end
        tmp= expX(D+1) *exp(-0.5*tmp);
        Q(i,j)=tmp;
    end
end
Q = Q + expX(D+2)*eye(n);
